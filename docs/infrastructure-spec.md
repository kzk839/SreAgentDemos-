...
| VPN Gateway × 2 | `{prefix}-vpngw-onprem`, `vpngw-hub` | VpnGw1 SKU, RouteBased, BGP 無効 |
...
| Azure Container Registry | `{prefix}acr` | コンテナイメージ格納。Private Endpoint 経由でアクセス |
...
| GitHub Actions Workflow | `.github/workflows/deploy.yml` | ビルド → ACR プッシュ → Container Apps デプロイ |
| Federated Credential | Entra ID | GitHub Actions → Azure 認証（OIDC、シークレット不要） |
...
| `vmSize` | string | `Standard_B2s` | 全 VM のサイズ |
...
# 4. サンプルアプリのコンテナイメージをビルド & プッシュ（初回のみ）
az acr build --registry <acr-name> --image sre-demo-app:latest ./app/

# 5. GitHub リポジトリに Federated Credential を設定（初回のみ）
# → GitHub Actions から OIDC で Azure に認証し、ACR プッシュ + Container Apps デプロイ
```
...
- [ ] Azure Bastion の追加（VM アクセス用）
- [ ] OIDC Federated Credential 設定スクリプト（GitHub Actions 用）
- [ ] SRE エージェント連携設定
