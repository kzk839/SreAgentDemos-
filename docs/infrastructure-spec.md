...
| `/chaos/latency` | POST | 全リクエストに遅延注入 | `ms` (最大 30000), `seconds` (最大 3600) |
| `/chaos/error` | POST | 全リクエストに 500 エラー返却 | `seconds` (最大 3600) |
...
- **遅延値は閾値の 2 倍以上を推奨:** `app-slow-response` アラートの閾値は 5 秒です。カオス注入の遅延は 10 秒（`ms: 10000`）以上に設定してください。
...
- **リクエスト送信が必要:** カオス注入（`/chaos/latency` 等）は後続のリクエストに効果を適用するため、カオス注入と並行して対象パス（例: `/`）にリクエストを送信するループが必要です。
- **VNet 内部アクセス:** Container App は `internal: true` のため、VNet 内の VM（Azure Bastion 経由）からリクエストを送信してください。

```powershell
# カオス注入（1回実行、1時間有効、ループ不要）
Invoke-RestMethod -Uri "https://<FQDN>/chaos/latency" -Method POST -ContentType "application/json" -Body '{"ms": 10000, "seconds": 3600}'

# リクエスト送信ループ（別ウィンドウで実行）
while ($true) {
